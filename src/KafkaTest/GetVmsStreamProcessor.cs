...
namespace KafkaTest {
    public class GetVmsStreamProcessor : IDisposable {
        IProducer<string, string> _producer;
        IConsumer<string, string> _consumer;
        private string _inputTopic;
        private string _outputTopic;
        public GetVmsStreamProcessor(string bootstrapServers, string inputTopic, string outputTopic)
        {
        ...
        private async Task RunInternal()
        {
            while(true)
            {
                ...
                var input = JsonConvert.DeserializeObject<GetVmsStreamRequest>(message.Value);
                var streamUrl = $"rtsp://192.168.0.111/{input.CameraId}-{DateTime.Now.Ticks}";
                Console.WriteLine($"GetVmsStreamProcessor Input {input.CameraId}");
                var value = new GetVmsStreamResult { CameraId = input.CameraId, StreamUrl = streamUrl, RequestId=input.RequestId };
                var valueJson = JsonConvert.SerializeObject(value);

                await _producer.ProduceAsync(_outputTopic, new Message<string, string> { Value = valueJson });
                _consumer.Commit();
                Console.WriteLine($"GetVmsStreamProcessor Output {valueJson}");
            };
